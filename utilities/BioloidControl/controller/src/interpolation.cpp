...
void doWriteData(struct RobotData &robotData, struct InterpolationData &ipoData, struct SCurveParameters &params)
{
	...
	{
		case IPO_PTP:
		writePositionDataPTP(ipoData.ipoPause, robotData.writeStartBuffer, &robotData.writeBuffer[robotData.writeBufferIndex][0], (float)ipoData.ipoCounter/(float)ipoData.ipoMax, params.tb, params.te, params.sgn, params.vmax, params.bmax); 
		break;
					
		case IPO_PTP_SINE:
		writePositionDataPTPSinus(ipoData.ipoPause, robotData.writeStartBuffer, &robotData.writeBuffer[robotData.writeBufferIndex][0], (float)ipoData.ipoCounter/(float)ipoData.ipoMax, params.tb, params.te, params.sgn, params.vmax, params.bmax); 
		break;
		
		case IPO_LINEAR:
		writePositionData(robotData.writeStartBuffer, &robotData.writeBuffer[robotData.writeBufferIndex][0], ipoData.ipoCounter, ipoData.ipoMax);
		break;
	}
}
...
byte doInterpolation(struct RobotData &robotData, struct InterpolationData &ipoData, struct SCurveParameters &params)
{
	...
	if (tmpTime >= robotData.readTime + ipoData.ipoPause)
	{
		...
		if (ipoData.ipoCounter <= ipoData.ipoMax)
		{
			...
			if ((robotData.writeBufferLength > ipoData.ipoParam) && ipoData.preparationDone)
			{
				...
				*/
				...
				doWriteData(robotData, ipoData, params);
		...
		//if (ipoData.ipoCounter >= ipoData.ipoMax)
		{
			...
			if (!ipoData.preparationDone) // comment this line in order to produce position feedback during write actions
				readPositionData(robotData.readBuffer, failure);
...
void writePositionData(byte* readBuffer, byte* writeBuffer, long int pos, long int max)
{
...
// ptp movement (constant acceleration)
void writePositionDataPTP(unsigned long  readPause, byte* readBuffer,byte* writeBuffer, float pos, float* tb, float* te, int* sgn, float* vmax, float* bmax)
{
	int i;
	long int current;
	float se, t;
